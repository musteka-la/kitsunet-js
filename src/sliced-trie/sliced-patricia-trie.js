...
class SlicedPatriciaTrie extends FakeTree {
  constructor ({ root, depth, prefixLen, sliceManager }) {
    super()

    assert(sliceManager, 'SliceStore is required!')
  ...
  get (key, cb) {
    ...
    this._sliceManager.getSliceById(sliceId).then((slice) => {
      if (slice) {
        const [node, rest] = utils.findNode(key, slice, this._prefixLen)
        if (!node && rest > 0) {
